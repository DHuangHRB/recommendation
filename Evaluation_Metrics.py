...
def Evaluation(Recommend, train, test, K, N):
    hit = 0
    all_tu = 0
    all_ru = 0
...
def Recall(Recommend, train, test, K, N):
    hit = 0
    all = 0
    for user in train.keys():
        tu = test[user].keys()
...
def Precision(Recommend, train, test, K, N):
    hit = 0 
    all = 0
    for user in train.keys():
        tu = test[user].keys()
...
def Coverage(Recommend, train, test, K, N):
    recommmend_items = set()
    all_items = set()
    for user in train.keys():
        for item in train[user].keys():
...
def Popularity(Recommend, train, test, K, N):
    item_popularity = dict()
    for user, items in train.items():
        for item in items.keys():
            if item not in item_popularity:
